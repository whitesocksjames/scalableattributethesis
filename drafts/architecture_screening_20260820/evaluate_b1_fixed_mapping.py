#!/usr/bin/env python3
"""Experiment-only B1 fixed step=4/8 evaluation over a manifest shard."""
import argparse
import csv
import os
import time
...
FIELDS = (
    "sample", "model_id", "partition_id", "points", "step",
    "prefix_bits", "qB_bits", "qE_bits", "base_bits", "full_bits",
    "official_r02_bits", "base_bpp", "full_bpp", "official_r02_bpp",
    ...
    "layered_over_original_ratio", "full_max_abs_difference",
    "native_capture_max_abs_difference", "seconds",
)
...
def quality(output_dir, label, coords, rgb, reconstruction):
    ...
    gt_path = os.path.join(directory, "gt.ply")
    rec_path = os.path.join(directory, "rec.ply")
    ...
    write_ply_ascii(
        rec_path, reconstruction.C[:, 1:].detach().cpu().numpy(), rec_rgb)
...
def main():
    args = parse_args()
    for name in ("data_root", "file_list", "r02_checkpoint", "gpcc_binary",
                 "output_dir"):
    ...
    adapter = BaseAdapter(
        args.r02_checkpoint, scale=5, stage=1, vmode=1).cuda().eval()
    rows = []
    for local_index, entry in enumerate(entries):
        ...
        official_bits = prefix_bits + len(encoded[4]["strings"]) * 8
        context = b1.decoded_r5_context(
            adapter.base, A, x_low, encoded, 16384)
        official_full = adapter.base.decode(
            context["x0"], x_low, encoded, lmb=16384)
        native_from_q = common.synthesize(
        ...
        if native_difference != 0:
            raise RuntimeError("Native capture differs from official decode")
        ...
        for step in (4, 8):
            ...
            base_reconstruction = common.synthesize(
                adapter.base,
                common.sparse(coarse, context["symbols"]), context)
            full_reconstruction = common.synthesize(
                adapter.base,
                common.sparse(recovered, context["symbols"]), context)
            full_difference = float(
                (full_reconstruction.F - official_full.F).abs().max().item())
            if full_difference != 0:
                raise RuntimeError("Layered Full differs from official R02 Full")
            base_quality = quality(
                args.output_dir, "base_{:04d}_s{}".format(global_index, step),
                coords, rgb, base_reconstruction)
            qB_bits = len(qB_string) * 8
            qE_bits = len(qE_string) * 8
            base_bits = prefix_bits + qB_bits
            full_bits = base_bits + qE_bits
            rows.append({
                "sample": entry, "model_id": model_id,
                "partition_id": partition_id, "points": len(A), "step": step,
                "prefix_bits": prefix_bits, "qB_bits": qB_bits,
                "qE_bits": qE_bits, "base_bits": base_bits,
                "full_bits": full_bits, "official_r02_bits": official_bits,
                "base_bpp": base_bits / len(A), "full_bpp": full_bits / len(A),
        ...
        del A, encoded, x_low, context, official_full, native_from_q
