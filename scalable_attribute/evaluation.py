...
def sample_identity(manifest_entry):
    ...
    if len(path.parts) != 2 or not _MODEL.fullmatch(path.parts[0]):
        raise ValueError("Expected manifest entry RWTxxx/name_Pn.h5: " + manifest_entry)
    match = _PARTITION.fullmatch(path.name)
    if not match:
        raise ValueError("Expected partition filename *_Pn.h5: " + manifest_entry)
...
def aggregate_models(rows, bits_field="base_bits", metric_prefix=""):
    ...
    output = []
    for model_id in sorted(grouped, key=lambda value: int(value[3:])):
